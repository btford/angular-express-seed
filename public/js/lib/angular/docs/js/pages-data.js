...
angular.module('pagesData', [])
  ...
  "api/ng/directive/ngSubmit": {
    ...
    "searchTerms": {
      ...
      "keywords": "$event action additionally alert alert-warning angular api attributes binding careful class current data-action default detailed directive discussion documentation double-submission enables eval event events example-example32 expression expressions form guide handlers js js-angular-release ng ngclick ngeventdirs ngsubmit object onsubmit prevents reloading request sending server src submitting-a-form-and-preventing-the-default-action triggered x-action"
    }
  },
  ...
  "error/$injector/unpr": {
    ...
    "searchTerms": {
      ...
      "keywords": "$injector accidentally angular api caused code controller correctly created define defined definition dependency destroyed directive docs entire error example fail fixed function js-angular-release making module myapp mycontroller mycoolservice mydirective mymodule myservice ngdoc problem project provider redefining required resolve retrieve service spelled subsequent syntax throw throws unable unknown unpr"
    }
  },
  ...
  "tutorial/step_02": {
    ...
    "searchTerms": {
      ...
      "keywords": "$controller $rootscope $scope __ __controller__ __model__ __template__ __view__ add adding alert alert-info allows angular angular-seed angularjs anonymous app application appropriate apps array assigned attaches attribute automatically background beforeeach behavior-driven bind binding bindings bootstrapping bower_components braces browser called case change changes changing chrome class code component components concept concerns connected constructed constructs contained context controller controllers create created critical crucial ctrl curly data data-binding declared decouple defined demonstrates denote descendant describe design developed developers development diagram directive doc-tutorial-nav doc-tutorial-reset docs documentation don dots dynamic dynamically easy element encourage encouraged ensure establish example executed execution expect experiments expressions extremely fail fast faster features file files framework full function functions generated generation github global glue going guide hard-coded hello html http ignore img incrementing info inject injected install installed instance instantiate instantiated instantiates io isn jasmine javascript js js-angular-release karma learn length limited list literal ll load located logic memory mind minimize minimized mock model model-view-controller models module motorola namespace nexus ng ng-app ng-controller ng-repeat ngcontroller ngdoc ngrepeat non-global notation note notice npm number object occur opened org os output parameter passed pattern phone phonecatapp phonelistctrl phones plays plugins png point points practice pre-configured prefer presentation previous project projection property prototypical providing records references referring reflect reflected refresh refreshes registered repeater replaced require rerun retrieve role running scope scopes search secs separate separating server service set simple simply slow snippet socket software source specifies src start started starting step step_02 step_03 structure success summary sweet sync syntax table tablet tag takes tells template templates terminal test testing tests text three time tobe tpum9dxclhtztkbaeo-n tutorial unit update updates v1 var verifies verify view views watch ways web wi-fi wikipedia work world wrapped write writing wrote xoom yay"
    }
  },
  "tutorial/step_03": {
    "docType": "tutorial",
  ...
  "tutorial/step_10": {
    ...
    "searchTerms": {
      ...
      "keywords": "$controller $http $httpbackend $new $rootscope $routeparams $scope __ add addition alert angular app appropriate appropriately attribute beforeeach better bound button change changed class click clickable clicked clicking clicks controller controllers create created css ctrl current data declared default describe desired detail details diagram directive display displays doc-tutorial-nav doc-tutorial-reset docs element elmo end-to-end event expect expectget experiments feature fetch flush function getattribute great handler handlers hello html image images imageurl img inherited jpg js js-angular-release json large learn li ll main mainimageurl method methods model module move ng ng-click ng-repeat ng-src ngclick ngdoc ngsrc npm operational order pass passing phone phone-detail phone-list phone-thumbs phonecatapp phonecatcontrollers phonecatctrl phonedetailctrl phoneid phonelistctrl phones place png property protractor ready refactor registered remains replace rerun respond return returns scope second set setimage smaller snippet src step step_10 step_11 style success summary swap swapper template test tests thumbnail thumbnails tobeundefined todo toequal tomatch tutorial unit url user var verifies verify view working world xyz xyzphonedata"
    }
  },
  "tutorial/step_11": {
    "docType": "tutorial",
